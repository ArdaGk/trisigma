...
class Backtest_Trail:
    """This class can be used to create trailling loss orders"""
    def __init__(self, broker, perc, interval='5m', save=False):
        """Constructor
        :param broker: broker object
        :param perc: The trailling percentage (0.0-1.0)
        :param save: Whether to save the trail loss in a list for plotting:
    ...
    def __call__(self):
        ...
        if self.active:
            if not self.locked:
                price = self.broker.get_ohlc(self.interval)[0]['high']
                cond1 = price * self.perc * self.dir < self.trail * self.dir
                cond2 = self.trail == -1
                if cond1 or cond2:
                    self.last_trail = self.trail
    ...
    def reset(self, target_price=None):
        ...
        perc = self.perc
        if target_price == None:
            self.trail = self.broker.get_ohlc(self.interval)[0]['high'] * perc
        else:
            self.trail = target_price * perc
    ...
    def on_hit(self):
        """Returns true if the price hit the trail"""
        price = self.broker.get_ohlc(self.interval)[0]['low']
        return self.active and price <= self.trail
